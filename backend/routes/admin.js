...
router.get('/dashboard', [auth, authorize('admin')], async (req, res) => {
  try {
    ...
    const totalVolume = completedContractsData.reduce((sum, contract) => sum + contract.terms.totalValue, 0);
    const platformRevenue = totalVolume * 0.025; // 2.5% commission
    ...
    res.json({
      ...
      data: {
        ...
        systemHealth: {
          ...
          activeConnections: totalUsers // Simplified
        }
      }
    });
  } catch (error) {
...
router.get('/transactions', [auth, authorize('admin')], async (req, res) => {
  try {
    ...
    // Transform contracts to transaction format
    const transactions = contracts.map(contract => ({
      ...
      commission: contract.platformFee.amount || (contract.terms.totalValue * 0.025),
